...
public class ArrayUtil {
   ...
   public static int[] getSortOrder(ArrayList<Double> da) {
	   int np = da.size();
	   ArrayList<DblIdx> dbal = new ArrayList<DblIdx>();
	   for (int i = 0; i < np; i++) {
		   double d = da.get(i).doubleValue();
		   DblIdx didx = new DblIdx(d, i);
		   dbal.add(didx);
	   }
	   Collections.sort(dbal);
	   int[] ret = new int[np];
	   double dpr = 0.;
	   for (int i = 0; i < np; i++) {
		   ret[i] = dbal.get(i).idx;

		   double dnx = dbal.get(i).dbl;
		   if (i > 0 && dnx < dpr) {
			   E.error("sort order failed");
		   }
		   dpr = dnx;
	   }
	   return ret;
