...
/**
 * divided into segments either with fixed lengths or with equal integral of the
...
public class TreeDiscretizer {
	...
	public TreeDiscretizer(TreePoint[] sp) {
		srcPoints = sp;
	...
	private void discretize() {
		...
		// dont do them in the loop above because it could mess up the search
		// for
		// later ones
	...
	/*
	public CompartmentTree getCompartmentTree(double l2r, int nmax) {
		TreePoint[] tpa = getBalancedSlices(Math.sqrt(l2r), nmax);

		Compartmentizer cptz = new Compartmentizer(tpa);

		return cptz.makeCompartmentTree();
	}
*/
