	...
	Prop *_prop;
{
	...
	double *_p; Datum *_ppvar;
 	_p = (double *)ecalloc(12, sizeof(double));
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
 	...
 	ivoc_help("help ?1 na3 /home/rcc/PSICS/eclipse/Examples/src/org/psics/examples/migca1/nrn/i386/na3.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
 static double _lmexp , _lhexp , _lsexp ;
static int _reset;
...
static int  trates (  _lvm , _la2 )  
 ...
 _lqt = pow( q10 , ( ( celsius - 24.0 ) / 10.0 ) ) ;
   ...
   taus = bets (  _lvm ) / ( a0s * ( 1.0 + alps (  _lvm ) ) ) ;
   if ( taus < smax ) {
     taus = smax ;
     }
    return 0; }
